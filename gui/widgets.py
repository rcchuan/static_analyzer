...
class SliceDialog:
    """Dialog for program slicing parameters"""

    def __init__(self, parent, theme: Dict):
        self.result = None
        self.theme  = theme
        self.dialog = tk.Toplevel(parent)
        self.dialog.title("程序切片 - 切片准则")
    ...
    def _build(self):
        theme = self.theme
        tk.Label(self.dialog, text="🔪 静态后向切片",
        ...
        tk.Label(self.dialog,
                 ...
                 font=("Segoe UI", 9), justify=tk.CENTER).pack(pady=(0, 16))

        form = tk.Frame(self.dialog, bg=theme["bg_dark"])
        form.pack(padx=20, fill=tk.X)

        tk.Label(form, text="行号:", bg=theme["bg_dark"],
                 fg=theme["text"], font=("Segoe UI", 10),
                 width=8, anchor=tk.W).grid(row=0, column=0, pady=4)
        self.line_var = tk.StringVar()
        tk.Entry(form, textvariable=self.line_var,
                 bg=theme["bg_mid"], fg=theme["text"],
                 insertbackground=theme["text"],
                 relief=tk.FLAT, font=("Consolas", 10)
                 ).grid(row=0, column=1, sticky="ew", padx=8)

        tk.Label(form, text="变量名:", bg=theme["bg_dark"],
                 fg=theme["text"], font=("Segoe UI", 10),
                 width=8, anchor=tk.W).grid(row=1, column=0, pady=4)
        self.var_var = tk.StringVar()
        tk.Entry(form, textvariable=self.var_var,
                 bg=theme["bg_mid"], fg=theme["text"],
                 insertbackground=theme["text"],
                 relief=tk.FLAT, font=("Consolas", 10)
        ...
        tk.Button(btn_frame, text="计算切片", command=self._ok,
                  bg=theme["accent"], fg="white",
                  font=("Segoe UI", 10, "bold"), relief=tk.FLAT,
                  padx=16, pady=6, cursor="hand2").pack(side=tk.LEFT, padx=8)
    ...
    def _ok(self):
        try:
            line = int(self.line_var.get().strip())
            var  = self.var_var.get().strip()
            if var:
                self.result = (line, var)
                self.dialog.destroy()
        except ValueError:
            pass
